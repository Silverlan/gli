	...
	inline texture::size_type compute_ktx_storage_size(texture const & Texture)
	{
		...
		for(texture::size_type Level = 0, Levels = Texture.levels(); Level < Levels; ++Level)
		{
			...
			for(texture::size_type Face = 0, Faces = Texture.faces(); Face < Faces; ++Face)
			{
				...
				texture::size_type const PaddedSize = std::max(BlockSize, glm::gtc::ceilMultiple(FaceSize, static_cast<texture::size_type>(4)));
	...
	inline bool save_ktx(texture const& Texture, std::vector<char>& Memory)
	{
		...
		for(texture::size_type Level = 0, Levels = Texture.levels(); Level < Levels; ++Level)
		{
			...
			for(texture::size_type Face = 0, Faces = Texture.faces(); Face < Faces; ++Face)
			{
				...
				texture::size_type const PaddedSize = glm::gtc::ceilMultiple(FaceSize, static_cast<texture::size_type>(4));
			...
			ImageSize = glm::gtc::ceilMultiple(ImageSize, static_cast<std::uint32_t>(4));
		}
